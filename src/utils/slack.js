 ...
 * Step 1: posts in the original thread only.
 * Steps 2 & 3: forwards to the escalation channel only (with forwarded-message visual).
 */
async function postReminder(client, channelId, threadTs, step) {
  ...
  // Steps 2 and 3: forward to the escalation channel with a Slack-style forwarded visual
  if (step >= 2) {
    try {
      ...
      // Embed timestamp using Slack date token to control ordering
